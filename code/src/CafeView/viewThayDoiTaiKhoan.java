...
import CafeController.controllerThayDoiThongTin;
import database.JDBCUtil;

public class viewThayDoiTaiKhoan extends JFrame {
	...
	public viewThayDoiTaiKhoan(String tenTaiKhoan, String matKhau, String chucVu) {
		...
		ActionListener action = new controllerThayDoiThongTin(this);

		JLabel lb_dangKyTaiKhoan = new JLabel(" THAY ĐỔI THÔNG TIN TÀI KHOẢN");
		lb_dangKyTaiKhoan.setIcon(new ImageIcon(viewThayDoiTaiKhoan.class.getResource("/icon/change_account.png")));
		...
		if (chucVu.equals("Quản lý")) {
			rdbtn_quanLy.setSelected(true);
		} else if (chucVu.equals("Nhân viên")) {
			rdbtn_nhanVien.setSelected(true);
		}
	...
	public void xacNhan(String tenTaiKhoanCu) {
		String tenTaiKhoanMoi = tF_tenTaiKhoan.getText();
		String matKhau = tF_matKhau.getText();
		String rdbtn_value = btnGroup.getSelection().getActionCommand();
		if ((tenTaiKhoanMoi.isEmpty() || matKhau.isEmpty()) || (tenTaiKhoanMoi.isEmpty() && matKhau.isEmpty())) {
			...
			String update = "UPDATE taikhoan SET tenTaiKhoan = '" + tenTaiKhoanMoi + "', matKhau = '" + matKhau
					+ "', chucVu = '" + rdbtn_value + "' WHERE tenTaiKhoan = '" + tenTaiKhoanCu + "'";
			try (Connection cn = JDBCUtil.getConnection(); PreparedStatement stmt = cn.prepareStatement(update)) {
				int rowsInserted = stmt.executeUpdate();
				if (rowsInserted > 0) {
					JOptionPane.showMessageDialog(this, "THAY ĐỔI THÔNG TIN TÀI KHOẢN THÀNH CÔNG!", "THÔNG BÁO!",
